...
import { topicDiversityRules } from './plan.js';
...
export function buildPrompts(ctx) {
  const {
    ...
    isFinalExam = false,  // engages stricter topic-spread rule for the plan
  ...
  // Topic-diversity thresholds — single source of truth in lib/plan.js so
  // the prompt's rule and the validator's check cannot drift. Final Exam
  // engages the stricter spread (≥ 6 distinct, ≤ 25% per topic) instead
  // of the standard ruleset (≥ 3 distinct, ≤ 50% per topic).
  const { minDistinctAtpEntries: minDistinctTopics, maxMarksPerAtpEntry: maxMarksPerTopic } =
    topicDiversityRules({ totalMarks, atpTopics, isFinalExam });
...
PARENT vs SUB-QUESTION MARK RULE — non-negotiable (F9 / Resource 5 prevention):
When a question has lettered sub-parts (a), (b), (c), the marks live ONLY on the sub-parts. The PARENT line that introduces the question MUST NOT carry its own (X) at end of line — that produces a duplicate count of the same marks (the parent's (X) PLUS the children's (X) marks both appear at line end and both get added together by the mark counter).

  WRONG (parent has its own (4) AND children also have marks — double-counted):
    12.2 Mr Petersen's method represents an old trading system. (4)
    (a) Name this trading system. (1)
    (b) Explain TWO reasons. (3)

  CORRECT (parent has NO (X), only children carry marks):
    12.2 Mr Petersen's method represents an old trading system.
    (a) Name this trading system. (1)
    (b) Explain TWO reasons. (3)

  ALSO CORRECT (parent on the same line as the FIRST sub-part — the (X) is sub-part (a)'s mark):
    12.1 (a) Identify the THREE different methods of payment. (3)
    (b) For each method, explain ONE specific advantage. (3)

If you want to show a question-block total, use the BLOCK form on its own line:
    [10]
Block totals are visual only and are NOT counted as marks.
...
- MCQ: ALWAYS write the question stem FIRST on its own line, ending with the (X) mark. THEN list ALL FOUR options a. / b. / c. / d. in order on consecutive lines. THEN one answer line. Never split options across the stem.
  ...
  MCQ ANSWER DISTRIBUTION — non-negotiable (F8 / Resource 5 prevention):
  Spread correct answers approximately evenly across options a / b / c / d.
  In any block of 8 or more MCQs the correct answer MUST appear at every
  one of a, b, c, d at least once. NEVER produce a paper where the correct
  answer is always 'b' or always 'c' — that pattern lets a guessing
  learner score 70%+ without reading the question. Before finalising,
  silently check that no single letter accounts for more than 50% of the
  correct answers in any MCQ section.
...
Return JSON: {"content":"memorandum table and TOTAL line only"}`;
  ...
  // Phase 3B prompt — refactored in Phase 1.1 of the structural pass.
  //
  // The cognitive-level analysis section is NO LONGER produced by Claude.
  // It is emitted deterministically in code after this call returns
  // (lib/cog-table.js → buildCogAnalysisTable). Claude only writes the
  // rubric and extension activity here.
  //
  // Reason: five audits found Claude breaking at least one of:
  //   - Prescribed Marks sums to the cover total
  //   - Actual Marks per level matches the answer rows
  //   - Actual % column sums to 100% (or matches actual/total)
  //   - Breakdown header "(X marks)" matches the listed-items sum
  //   - Breakdown trailing "= X marks" matches the listed-items sum
  // The deterministic emitter cannot violate any of these by construction.
  const mUsrB = (memoTable, actualTotal) => `Grade ${g} ${subject} — ${resourceType} — Term ${t}

Completed memorandum answer table (${actualTotal} marks total):
${memoTable}

YOUR TASK: Write ONLY the section(s) requested below. Do NOT write a
cognitive level analysis table — that is generated by code from the answer
table above and inserted automatically. Anything you produce in that area
will be discarded.
${includeRubric ? `

SECTION: MEMO MARKING RUBRIC — TEACHER USE ONLY
The rubric is a TEACHER reference for the WHOLE PAPER. It must NOT be
presented as if it applies to the extension activity that follows.
...
Return JSON: {"content":"${includeRubric ? 'rubric + ' : ''}${!isWorksheet ? 'extension' : ''}"}`;
