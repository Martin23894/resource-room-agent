 ...
 * the last option in a sequencing/ordering question — e.g. "E." placed
 * below "Answer:"), move them back to before the answer line so the
 * question reads correctly.
 *
 * The lookahead stops at the next sub-question header (SUBQ_WITH_MARKS) so
 * we never steal the following question's options.
 */
function reorderMisplacedOptions(lines) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    for (let j = i + 1; j < Math.min(lines.length, i + 7); j++) {
      if (SUBQ_WITH_MARKS.test(lines[j])) break; // next question — stop here
