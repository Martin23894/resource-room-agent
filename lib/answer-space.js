...
// Wider option pattern for sequencing questions: a-h covers the realistic
// range. The narrower HAS_MCQ_OPTION (a-d only) is intentionally kept for
// the standard MCQ injection path so we don't accidentally treat e.g. an
// "E. coli" passage line as an MCQ option.
const HAS_DISPLACED_OPTION = /^\s*[a-hA-H][.)]\s/;

/**
 * If Claude generated MCQ options AFTER the "Answer: ___" line, move them
 * back to before the answer line so the question reads correctly. This
 * covers two real failure modes from production:
 *
 *   1. Sequencing/ordering question with an "E." option dropped below the
 *      answer stub (the original case this was added for).
 *   2. The Grade 7 Nat Sci pattern: AI emits options a/b/c for Q(n+1)
 *      BEFORE Q(n+1)'s stem, then the answer stub for Q(n), then Q(n+1)'s
 *      stem, then Q(n+1)'s option d. The "d" is stranded after the stub
 *      AND after a stem that may wrap to multiple physical lines.
 *
 * The lookahead stops at the next numbered sub-question header
 * (SUBQ_WITH_MARKS) so we never steal a following numbered question's
 * options. A 14-line window clears blank padding plus a wrapped stem.
 */
function reorderMisplacedOptions(lines) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    for (let j = i + 1; j < Math.min(lines.length, i + 14); j++) {
      if (SUBQ_WITH_MARKS.test(lines[j])) break; // next numbered question — stop here
      if (HAS_DISPLACED_OPTION.test(lines[j])) {
...
export function ensureAnswerSpace(text, opts = {}) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    // Detection uses the narrow a-d pattern (so we don't mistake e.g. an
    // "E. coli" passage line for an MCQ block), but the LAST-OPTION search
    // uses the wider a-h pattern so a 5-option sequencing question places
    // its answer stub after E, not after D.
    if (HAS_MCQ_OPTION.test(ahead)) {
      let lastOptionIdx = -1;
      for (let j = i + 1; j < stopIdx; j++) {
        if (HAS_DISPLACED_OPTION.test(lines[j])) lastOptionIdx = j;
      }
