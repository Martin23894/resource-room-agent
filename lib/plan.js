...
/**
 * Topic-diversity thresholds. Single source of truth for the prompt
 * (lib/prompts.js → buildPrompts) AND the validator (planContext below)
 * — when these drift, the validator rejects plans the prompt said were
 * fine. Computing them once and reading from one source keeps them
 * synchronised.
 *
 * Final Exam mode: Resource 5 audit (Grade 7 EMS Final Exam Term 4)
 * shipped a 103-mark "final" covering only 1 of ~12 ATP topics across
 * all four terms. The non-Final ruleset (≥ 3 distinct, ≤ 50% per topic)
 * accepted that monoculture because 1 topic with 100% of marks doesn't
 * trip the cap. Final Exam needs a much stricter spread.
 */
export function topicDiversityRules({ totalMarks, atpTopics = [], isFinalExam = false }) {
  const list = Array.isArray(atpTopics) ? atpTopics.filter((t) => typeof t === 'string' && t.length) : [];
  if (isFinalExam) {
    // Final Exam covers all four terms. Require breadth proportional to
    // the available ATP entries — at least 6 distinct topics, or roughly
    // half the available list, whichever is larger. Capped at the number
    // of topics actually present so an unusually thin ATP doesn't make
    // the rule unsatisfiable.
    const minDistinct = Math.min(list.length, Math.max(6, Math.ceil(list.length / 2)));
    // Cap per-topic at 25% of marks. On a 100-mark paper that's 25 marks
    // max per ATP entry — enough room for a substantial sub-section but
    // not for the single-topic monoculture Resource 5 shipped.
    const maxPerEntry = Math.max(1, Math.ceil(totalMarks * 0.25));
    return { minDistinctAtpEntries: minDistinct, maxMarksPerAtpEntry: maxPerEntry, atpTopics: list };
  }
  // Standard ruleset: ≥ 3 distinct topics, ≤ 50% per topic.
  return {
 ...
 * `isFinalExam` (optional) — engages the stricter Final Exam ruleset.
 */
export function planContext({ totalMarks, cog, cogMarks, cogTolerance, atpTopics = [], isFinalExam = false }) {
  const diversity = topicDiversityRules({ totalMarks, atpTopics, isFinalExam });
  return {
    totalMarks,
    cog,
    cogMarks,
    ...
    atpTopics: diversity.atpTopics,
    minDistinctAtpEntries: diversity.minDistinctAtpEntries,
    maxMarksPerAtpEntry: diversity.maxMarksPerAtpEntry,
    isFinalExam,
  };
}
