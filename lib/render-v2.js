...
function renderStimulus(stimulus) {
  ...
  switch (stimulus.kind) {
    ...
    case 'visualText':
    case 'diagram': {
      // We cannot generate images. Render the verbal description in an
      // outlined block so it visually reads as a stimulus, not a question.
...
export function renderQuestionPaper(resource) {
  ...
  // Sections — inject each referenced stimulus once, the first time it's used.
  ...
  for (const section of resource.sections) {
    ...
    // Also catch question-level stimulusRefs that the section didn't declare.
  ...
  // Paper total — right-aligned green line.
...
// Exposed for tests / other renderers (Day 4 memo will reuse these).
