...
import { parseMemoAnswerRows, correctCogAnalysisTable } from './cog-table.js';
...
// ============================================================
// FIX 6 — Cognitive analysis "Actual Marks" repair
// ============================================================
//
// The memo's cognitive-analysis table has per-level rows whose
// "Actual Marks" cells must equal the sum of memo answer-table rows
// tagged with that level. After memo regen fixes individual mark cells,
// the cog table's Actual cells often go stale, producing the
// cog_breakdown_actuals failure. correctCogAnalysisTable recomputes
// these cells from the live answer-row tally — pure arithmetic, no
// Claude needed.

export function repairCogAnalysisActuals(ctx) {
  if (!ctx?.memo) return null;
  const levels = ctx?.cog?.levels;
  if (!Array.isArray(levels) || levels.length === 0) return null;
  const result = correctCogAnalysisTable(ctx.memo, {
    levels,
    totalMarks: ctx.totalMarks,
  });
  if (!result?.changed) return null;
  return { memo: result.text };
}
...
const FIXES = [
  ...
  //   5. Repair cog analysis "Actual Marks" FIFTH — runs after memo mark
  //      repair so the Actual cells reflect the corrected mark column.
  //   6. MCQ shuffler LAST — sees the fully corrected memo.
  ...
  { id: 'cog_analysis_actuals', applies: repairCogAnalysisActuals },
