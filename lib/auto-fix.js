...
const FIXES = [
  // Order matters:
  //   1. Strip phantom rows FIRST so subsequent fixes don't waste
  //      effort on rows that are about to be deleted.
  //   2. Repair memo marks SECOND so the mark column is correct
  //      before any cross-checks run downstream.
  //   3. MCQ shuffler LAST so it sees the post-clean memo.
