...
import { lookupImage } from './images/catalogue.js';
...
// STIMULI
// ─────────────────────────────────────────────────────────────────
function renderStimulus(stimulus, imageBytes) {
  const els = [];
  ...
  switch (stimulus.kind) {
    case 'image': {
      // Curated photograph from lib/images/catalogue.js. The bytes were
      // pre-fetched by api/generate.js before calling renderResource;
      // we just embed them here. Falls back to caption-only if the
      // imageId is unknown or the fetch failed (graceful degradation).
      const entry = lookupImage(stimulus.imageId);
      const bytes = imageBytes?.get?.(stimulus.imageId);
      if (entry && bytes) {
        els.push(new Paragraph({
        ...
        const caption = stimulus.caption || entry.caption;
        if (caption) els.push(para(caption, { italics: true, color: GREY, align: AlignmentType.CENTER, spaceAfter: 120 }));
      } else {
        // Fallback: text caption + altText so the paper still ships.
        const caption = stimulus.caption || entry?.caption || stimulus.altText || '[Photograph unavailable]';
        els.push(para(`[Photograph: ${caption}]`, { italics: true, color: GREY, spaceAfter: 80 }));
      }
      els.push(blank(40));
      break;
    }
...
// ENTRY POINT
// ─────────────────────────────────────────────────────────────────
export function renderQuestionPaper(resource, opts = {}) {
  const L = i18n(resource.meta.language);
  return buildDocument(resource, L, buildPaperContent(resource, L, opts.imageBytes));
}

export function renderResource(resource, opts = {}) {
  const L = i18n(resource.meta.language);
  const paper = buildPaperContent(resource, L, opts.imageBytes);
  paper.push(new Paragraph({ children: [new PageBreak()] }));
  return buildDocument(resource, L, [...paper, ...buildMemoContent(resource, L)]);
}

function buildPaperContent(resource, L, imageBytes) {
  const els = [];
  ...
  for (const section of resource.sections) {
    for (const ref of section.stimulusRefs || []) {
      ...
      if (st) {
        els.push(...renderStimulus(st, imageBytes));
        renderedStimuli.add(ref);
      }
    }
    for (const q of section.questions) collectQuestionStimulusRefs(q, stimulusById, renderedStimuli, els, imageBytes);
...
function collectQuestionStimulusRefs(q, stimulusById, rendered, els, imageBytes) {
  ...
  if (Array.isArray(q.subQuestions)) {
    q.subQuestions.forEach((sq) => collectQuestionStimulusRefs(sq, stimulusById, rendered, els, imageBytes));
  }
}
