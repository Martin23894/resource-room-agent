...
export function renderResource(resource) {
  ...
  // Lesson layout: lesson-plan pages → page break → worksheet pages → page
  // break → memo pages. The worksheet IS the existing sections+memo, just
  // rendered after a teacher-facing lesson-plan section.
  if (isLesson) {
    ...
    return buildDocument(resource, L, [...plan, ...paper, ...buildMemoContent(resource, L)]);
  }
...
function renderLessonPhase(phase, LL) {
  const els = [];
...
function renderLessonPhases(resource, LL) {
  ...
  for (const p of phases) els.push(...renderLessonPhase(p, LL));
  return els;
}
...
export const __internals = {
  ...
  tallyByLevel, localiseCogLevel, buildLessonPlanContent, lessonLabels,
};
