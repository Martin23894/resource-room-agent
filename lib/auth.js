...
export function getOrCreateUserByEmail(email) {
  ...
  const existing = db.prepare('SELECT id, email, created_at, last_login_at FROM users WHERE email = ?').get(normalised);
  const now = Date.now();
  if (existing) {
  ...
  db.prepare('INSERT INTO users (id, email, created_at, last_login_at) VALUES (?, ?, ?, ?)').run(id, normalised, now, now);
  return { id, email: normalised, created_at: now, last_login_at: now };
}

export function getUserById(id) {
  if (!id) return null;
  const db = getDb();
  return db.prepare('SELECT id, email, created_at, last_login_at FROM users WHERE id = ?').get(id) || null;
}
