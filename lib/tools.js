...
export const memoVerifyTool = {
  ...
  input_schema: {
    ...
    properties: {
      ...
      errors: {
        type: 'array',
        description: 'Per-row memo errors.',
        items: {
          type: 'object',
          properties: {
            ...
            check: {
              type: 'string',
              enum: ['ARITHMETIC', 'PROFIT_LOSS', 'COUNT', 'ROUNDING'],
            },
      ...
      cogLevelErrors: {
        type: 'array',
        description: 'Cognitive-level totals that disagree between the memo table and the analysis table.',
        items: {
          type: 'object',
          properties: {
            level:           { type: 'string', description: 'Cognitive level label.' },
            foundInTable:    { type: 'string', description: 'Total currently shown in the analysis table.' },
            foundInAnalysis: { type: 'string', description: 'Sum of MARK values across rows for this level in the memo.' },
            fix:             { type: 'string', description: 'Concrete edit that resolves the mismatch.' },
          },
          required: ['level', 'foundInTable', 'foundInAnalysis', 'fix'],
        },
      },
    },
