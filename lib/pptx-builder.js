...
import { pickHero, pickIcon, pickStamp, illustrationToPng, pickPhoto, loadPhotoBuffer } from './illustrations/index.js';
...
/**
 * Rasterise an illustration SVG to a base64 data URL for pptxgenjs's
 * addImage(). Returns null on raster failure so callers can fall back to
 * the Phase B placeholder shapes without breaking generation.
 */
function illustrationDataUrl(svg, widthPx = 600) {
  try {
    const png = illustrationToPng(svg, { widthPx });
...
// Photo sidebar geometry — used by concept / example / warmUp layouts
// for a consistent photo placement on the right side of the slide.
const PHOTO_BOX = Object.freeze({
  x: 9.4,                  // right edge of the slide minus margin
  y: 1.9,                  // sits below the heading bar
  w: 3.3,
  h: 4.6,
  textRightEdge: 9.0,      // text-content cells must end before this so they don't run under the photo
});

/**
 * Add a photo on the right of the slide if one exists for this subject +
 * slot. Returns { added: true } when a real photo was embedded so the
 * caller can narrow its text content. Missing photos resolve to
 * { added: false } and the caller renders full-width — no broken-image
 * placeholder, so a freshly-deployed library without curated photos still
 * produces a clean deck.
 */
function tryAddPhotoSidebar(slide, resource, slotIdx) {
  const photo = pickPhoto({
    subject: resource?.meta?.subject,
    slot:    slotIdx,
  });
  if (!photo || !photo.exists) return { added: false };

  try {
    const buffer = loadPhotoBuffer(photo);
    if (!buffer) return { added: false };
    const dataUrl = `data:image/jpeg;base64,${buffer.toString('base64')}`;
    slide.addImage({
      data: dataUrl,
      x: PHOTO_BOX.x, y: PHOTO_BOX.y, w: PHOTO_BOX.w, h: PHOTO_BOX.h,
      sizing: { type: 'cover', w: PHOTO_BOX.w, h: PHOTO_BOX.h },
    });
    return { added: true };
  } catch {
    return { added: false };
...
function renderTitleSlide(slide, s, resource, style) {
  ...
  // Try to render a hero illustration on the right side of the hero card
  // (junior gets the mascot, senior gets the subject icon). Falls back
  // silently — the text-only title slide still works.
  ...
  try {
    ...
    heroDataUrl = illustrationDataUrl(hero.svg, 600);
  } catch {
    heroDataUrl = null;
  }

  // Layout split: text occupies the left, hero image sits on the right
  // when present. Without the hero, text spans the whole card.
  const textRightEdge = heroDataUrl ? SLIDE_W - 4.4 : SLIDE_W - 1.5;
  const textW = textRightEdge - 1.5;
  ...
  slide.addText(s.heading || resource.cover.subjectLine, {
    x: 1.5, y: 2.1, w: textW, h: 1.6,
    fontFace: FONT_HEAD, fontSize: 48, color: palette.ink, bold: true,
    align: heroDataUrl ? 'left' : 'center', valign: 'middle',
  });
  ...
  if (resource.lesson?.capsAnchor?.unitTopic) {
    slide.addText(resource.lesson.capsAnchor.unitTopic, {
      x: 1.5, y: 3.7, w: textW, h: 0.7,
      fontFace: FONT_BODY, fontSize: 20, color: palette.inkSoft,
      align: heroDataUrl ? 'left' : 'center', italic: true,
    });
  }

  // Accent strip under the title.
  const stripX = heroDataUrl ? 1.5 : SLIDE_W / 2 - 1.0;
  slide.addShape('roundRect', {
    x: stripX, y: 4.5, w: 2.0, h: 0.12,
  ...
  slide.addText(subtitle, {
    x: 1.5, y: 4.8, w: textW, h: 0.5,
    fontFace: FONT_BODY, fontSize: 16, color: palette.green, bold: true,
    align: heroDataUrl ? 'left' : 'center',
  });

  if (Array.isArray(s.bullets) && s.bullets.length) {
    slide.addText(bulletItems(s.bullets), {
      x: 1.5, y: 5.4, w: textW, h: 0.5,
      fontFace: FONT_BODY, fontSize: 14, color: palette.inkSoft, align: 'left',
    });
  }

  // Hero illustration on the right side of the card (junior: mascot,
  // senior: subject icon). Mascot SVG is 100×130; subject icons are 100×100.
  if (heroDataUrl) {
    const heroBoxW = 2.4;
    const heroBoxH = style.gradeBand === 'junior' ? 3.1 : 2.4;
    const heroX = SLIDE_W - 1.6 - heroBoxW;
    const heroY = 1.9 + (4.4 - heroBoxH) / 2;
    slide.addImage({ data: heroDataUrl, x: heroX, y: heroY, w: heroBoxW, h: heroBoxH });
  }
}
...
// ─────────────────────────────────────────────────────────────────
// PHASE D — TEACHING SLIDE WITH OPTIONAL PHOTO SIDEBAR
//
// Used by `concept` and `example` layouts. Tries to embed a photo on
// the right (cf. tryAddPhotoSidebar) and narrows the bullet text area
// when a photo lands. Falls back to the plain renderBulletSlide layout
// when no photo is available, so a freshly-deployed library without
// curated photos produces clean text-only slides.
// ─────────────────────────────────────────────────────────────────

function renderTeachingSlide(slide, s, resource, style) {
  const { added } = tryAddPhotoSidebar(slide, resource, s.ordinal);
  if (!added) return renderBulletSlide(slide, s, resource, style);
  ...
  if (bullets.length === 0) {
    slide.addText('(No bullets supplied)', {
      x: 0.7, y: 2.0, w: PHOTO_BOX.textRightEdge - 0.7, h: 0.5,
      fontFace: FONT_BODY, fontSize: 16, color: style.palette.grey, italic: true,
    });
    return;
  }

  const fontSize = bullets.length <= 4 ? 22 : bullets.length <= 6 ? 18 : 16;
  slide.addText(bulletItems(bullets), {
    x: 0.7, y: 2.0, w: PHOTO_BOX.textRightEdge - 0.7, h: SLIDE_H - 3.0,
    fontFace: FONT_BODY, fontSize, color: style.palette.ink,
    paraSpaceAfter: 6, align: 'left', valign: 'top',
  });
}
...
function renderWarmUpSlide(slide, s, resource, style) {
  ...
  // Photo sidebar (Phase D) lands on the right when a photo is curated,
  // and the tinted card narrows to share the slide width with it.
  const palette = style.palette;
  const accent = nthAccent(style, 0);

  const { added } = tryAddPhotoSidebar(slide, resource, s.ordinal);
  const cardX = 1.0;
  const cardW = added ? (PHOTO_BOX.textRightEdge - cardX) : (SLIDE_W - 2.0);
  const cardY = 1.6;
  const cardH = 4.6;
  ...
  slide.addText('WARM-UP', {
    x: cardX + 0.5, y: cardY + 0.25, w: cardW - 1.0, h: 0.4,
  ...
  slide.addText(String(s.heading || ''), {
    x: cardX + 0.5, y: cardY + 0.8, w: cardW - 1.0, h: questionH,
    fontFace: FONT_HEAD, fontSize: added ? 28 : 36, color: palette.ink, bold: true,
    align: 'left', valign: 'top',
  });
  ...
  if (bullets.length > 0) {
    slide.addText(bulletItems(bullets), {
      x: cardX + 0.7, y: cardY + 3.1, w: cardW - 1.4, h: 1.4,
  ...
  slide.addShape('ellipse', {
    x: cardX + cardW - 0.6, y: cardY + 0.2, w: 0.5, h: 0.5,
    fill: { color: accent }, line: { color: accent },
  });
}
...
function renderYourTurnSlide(slide, s, _resource, style) {
  ...
  // Subject icon on the right of the panel — real SVG raster from
  // lib/illustrations/. Falls back to the original accent-square + play
  // arrow placeholder when raster is unavailable.
  ...
  try {
    const icon = pickIcon({
      subject: _resource.meta?.subject,
      accent:  `#${accent}`,
    });
    iconDataUrl = illustrationDataUrl(icon.svg, 600);
  } catch {
    iconDataUrl = null;
  }

  if (iconDataUrl) {
    slide.addImage({ data: iconDataUrl, x: iconX, y: iconY, w: iconSize, h: iconSize });
  } else {
    slide.addShape('roundRect', {
...
function renderCelebrateSlide(slide, s, resource, style) {
  // Closing well-done slide. Tinted bg + confetti-style decorative shapes
  // in the corners + a friendly headline + (junior) a "Well done!" stamp
  // image + a short recap list.
  const palette = style.palette;
  const language = resource.meta?.language || 'English';
  slide.background = { color: palette.bgTint };
  ...
  // Junior band gets the "Well done!" stamp image centered at the top —
  // larger visual hit than a text headline alone. Senior keeps the
  // text-only headline (cleaner / less "primary school").
  let stampDataUrl = null;
  if (style.gradeBand === 'junior') {
    try {
      const stamp = pickStamp({
        gradeBand: 'junior', language,
        accent:    `#${nthAccent(style, 0)}`,
      });
      if (stamp) stampDataUrl = illustrationDataUrl(stamp.svg, 700);
    } catch {
      stampDataUrl = null;
    }
  }
  ...
  if (stampDataUrl) {
    // Stamp on the left, recap on the right
    const stampSize = 2.6;
    slide.addImage({
      data: stampDataUrl,
      x: SLIDE_W / 2 - stampSize / 2, y: 1.7,
      w: stampSize, h: stampSize,
    });
    slide.addText(String(s.heading || 'Well done!'), {
      x: 1.0, y: 4.6, w: SLIDE_W - 2.0, h: 0.9,
      fontFace: FONT_HEAD, fontSize: 36, bold: true, color: palette.green,
      align: 'center', valign: 'middle',
    });
    const bullets = Array.isArray(s.bullets) ? s.bullets : [];
    if (bullets.length > 0) {
      slide.addText(bulletItems(bullets), {
        x: 2.5, y: 5.6, w: SLIDE_W - 5.0, h: 1.2,
        fontFace: FONT_BODY, fontSize: 18, color: palette.ink,
        paraSpaceAfter: 4, align: 'center', valign: 'top',
      });
    }
  } else {
    // Senior band (or stamp raster failure) — text-only celebrate.
    slide.addText(String(s.heading || 'Well done!'), {
...
const LAYOUT_RENDERERS = {
  ...
  // concept/example route through Phase D's teaching renderer so they
  // pick up the optional photo sidebar (cf. tryAddPhotoSidebar). When no
  // photo is curated for the slot, they fall back to the plain bullet
  // slide layout — no broken-image placeholder.
  concept:        renderTeachingSlide,
  example:        renderTeachingSlide,
...
export const __internals = {
  ...
  // Phase D teaching slide + photo helpers
  renderTeachingSlide, tryAddPhotoSidebar, PHOTO_BOX,
  // Plumbing + dispatch
