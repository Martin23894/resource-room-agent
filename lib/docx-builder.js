...
export function createDocxBuilder(config) {
  ...
  function parseText(text) {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      // Unwrap whole-line markdown bold (**TITLE**) so headings like
      // "**EXTENSION ACTIVITY**" classify as section heads instead of
      // rendering with literal asterisks.
      const boldWrap = tr.match(/^\*{1,3}(.+?)\*{1,3}\s*$/);
      if (boldWrap) tr = boldWrap[1].trim();
