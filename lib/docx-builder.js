  ...
  function parseText(text) {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (tr.includes('|') && tr.split('|').filter((c) => c.trim()).length >= 2) {
        ...
        // Strip markdown bold from each cell — Claude often writes
        // "**TOTAL** | **100%** | …" for the summary row of the cog-level
        // analysis table, which would otherwise render with literal asterisks.
        const stripMd = (c) => c.replace(/\*\*([^*]+)\*\*/g, '$1').replace(/\*\*/g, '');
        const parsed = rows.map((r) => r.split('|').map((c) => stripMd(c.trim())).filter((c) => c));
