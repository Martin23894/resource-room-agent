 ...
 *
 * Each unit also includes a `subtopicHeadings[]` list (top-level subtopic
 * headings only, no concept bullets) so the Lesson generator's frontend
 * can offer a Subtopic picker without a follow-up fetch. This keeps the
 * payload small (one Maths Gr 6 T2 unit averages ~6 headings × 60 chars).
 */
export function getPacingUnitsSlim(subject, grade, term) {
  ...
  return units
    ...
    .map((u) => ({
      ...
      subtopicHeadings: getUnitSubtopicHeadings(u),
    }));
}

/**
 * Extract the top-level subtopic-heading list from a Unit. Used by the
 * Lesson generator's Subtopic picker so the teacher can drill from a Unit
 * (CAPS-prescribed, ~3–8h block) into a specific concept (~45-min lesson).
 *
 * Drops empty headings and de-duplicates while preserving order. Children
 * (deeper-nested subtopics) are deliberately skipped — for v1 we only let
 * teachers pick at the top level. Returns [] when the Unit has no
 * structured subtopics (e.g. a revision block that's pure free-text).
 */
export function getUnitSubtopicHeadings(unit) {
  if (!unit || !Array.isArray(unit.subtopics)) return [];
  const seen = new Set();
  const headings = [];
  for (const st of unit.subtopics) {
    const h = (st?.heading || '').trim();
...
/**
 * Find a single top-level subtopic by exact heading match within a Unit.
 * Returns the Subtopic object (with its concepts + children) when found,
 * or null. Used by the generator pipeline to focus the prompt on the
 * concepts of just one subtopic instead of the whole Unit.
 */
export function findUnitSubtopic(unit, heading) {
  if (!unit || !heading || !Array.isArray(unit.subtopics)) return null;
  return unit.subtopics.find((st) => (st?.heading || '').trim() === heading.trim()) || null;
}
