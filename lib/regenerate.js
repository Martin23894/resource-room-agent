...
- Edit ONLY the parts the failures call out. Leave every other line character-for-character identical.
- Preserve every (X) mark value unless a failure explicitly requires changing it.
- Preserve every section [N] block total and the final TOTAL line UNLESS a failure requires updating them (e.g. cover_eq_body requires the section totals and TOTAL line to change too).
...
export async function runRegenerationLoop({
  ...
  for (const phase of REGEN_PHASES) {
    ...
    for (let attempt = 1; attempt <= effectiveCap; attempt++) {
      ...
      // Re-run deterministic auto-fixes after each regen attempt so that
      // structural repairs (section brackets, footer, memo marks) are
      // applied before the gate re-evaluates. This prevents cascades where
      // paper regen fixes individual (X) marks but leaves [N] brackets and
      // the TOTAL footer stale, causing the same structural failures to
      // reappear on the next gate check.
      const { ctx: autoFixed } = runAutoFixes(merged, { logger });
      currentCtx = autoFixed;
