...
export function openCache({ path = process.env.CACHE_DB_PATH, logger = defaultLogger } = {}) {
  ...
  `);
  const pruned = db.prepare('DELETE FROM cache WHERE expires_at IS NOT NULL AND expires_at < ?').run(Date.now()).changes;
  if (pruned > 0) logger.info?.({ pruned }, 'Cache: pruned expired entries on startup');
  return db;
}
