...
//       "COMBINED BARRETT'S TAXONOMY ANALYSIS" (RTT pipeline uses Barrett's framework), or
//   (b) starts with a pipe-table header row whose first cell is "Cognitive Level"
//       / "Kognitiewe Vlak" / "Barrett's Level" / "Barrett se Vlak" and the
//       second cell contains "Prescribed" / "Voorgeskrewe".
//
...
const COG_ANALYSIS_HEADING_RE = /(COGNITIVE\s+LEVEL\s+ANALYSIS|KOGNITIEWE\s+VLAK\s+ANALISE|BARRETT'?S\s+TAXONOM)/i;
const COG_ANALYSIS_TABLE_HEADER_RE = /\|\s*(Cognitive Level|Kognitiewe Vlak|Barrett'?s Level|Barrett se Vlak)\s*\|\s*(Prescribed|Voorgeskrewe)/i;
