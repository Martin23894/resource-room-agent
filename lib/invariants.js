...
// failure's category:
//
//   AUTO_FIX    — fixed deterministically in code elsewhere in the
//                 pipeline (e.g. ensureSequentialSectionLetters,
//                 correctCogAnalysisTable). The gate's job is to verify
//                 those fixes actually landed; a failure here means we
//                 shipped without the repair, which is a bug.
...
export function buildInvariantContext({
  ...
  totalMarks, cog, cogMarks, cogTolerance, pipeline,
}) {
  return {
...
// Block-total form: `[N]` at the end of a section. Used by section-bracket sum.
const SECTION_BLOCK_TOTAL = /\[(\d+)\]/g;
...
//
// These checks should already be true by the time the gate runs because the
// pipeline has dedicated phases (Phase 2a mark drift, Phase 4b cog table
// correction) that repair them. A failure here means a repair didn't land
// — surface as a hard error in enforce mode rather than silently shipping.
// ══════════════════════════════════════════════════════════════════════════
...
register({
  ...
  check(ctx) {
    ...
    const counted = countMarks(ctx.paper);
...
register({
  ...
  check(ctx) {
    ...
    const TOTAL_RE = /^(TOTAL|TOTAAL|TOTALE)\s*:?\s*(\d+)\b/i;
    let found = null;
...
// ══════════════════════════════════════════════════════════════════════════
// AUTO_FIX — LANGUAGE / LOCALISATION
// ══════════════════════════════════════════════════════════════════════════
...
// ══════════════════════════════════════════════════════════════════════════
// AUTO_FIX — MCQ HYGIENE (post-hoc balancing / pattern detection)
// ══════════════════════════════════════════════════════════════════════════
...
// ══════════════════════════════════════════════════════════════════════════
// AUTO_FIX — HYGIENE (meta-commentary / leakage)
// ══════════════════════════════════════════════════════════════════════════
...
register({
  ...
  check(ctx) {
    if (!ctx.memo) return null;
    // Bracketed editor notes inside table cells — "[Note: …]", "[TODO …]"
    const offenders = [];
    for (const line of ctx.memo.split('\n')) {
      if (!/^\s*\|/.test(line)) continue;
      if (/\[(Note|TODO|FIXME|TBD|Insert)\b/i.test(line)) {
        offenders.push(line.trim().slice(0, 100));
      }
    }
    if (offenders.length === 0) return null;
    return {
      message: `${offenders.length} memo cell(s) contain editor-style bracketed notes`,
      offenders,
      feedback: `Memo cells must contain real content, not editor notes like [Note: …] or [TODO …].`,
    };
  },
});
