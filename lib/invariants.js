...
register({
  ...
  check(ctx) {
    ...
    const needed = ctx.totalMarks - counted;
    const action = needed > 0
      ? `Add ${needed} mark${needed !== 1 ? 's' : ''}: increase the (X) value on one or more existing sub-questions, or add a new sub-question worth ${needed} marks.`
      : `Remove ${-needed} mark${-needed !== 1 ? 's' : ''}: decrease the (X) value on one or more existing sub-questions, or remove a sub-question.`;
    return {
      ...
      feedback: `The paper body sums to ${counted} marks but the cover declares ${ctx.totalMarks} marks (need ${needed > 0 ? '+' : ''}${needed}). ${action} After changing sub-question (X) values, also update each affected section [N] block total and the final TOTAL line so they all reflect the new sum.`,
    };
  },
});
