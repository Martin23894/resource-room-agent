...
export function stripLeadingMemoBanner(text) {
  ...
  if (i >= lines.length || !MEMO_BANNER.test(lines[i])) return text;
  ...
  while (i < lines.length) {
    ...
    // row, a numbered answer key row, or anything but a free-text title.
    if (/^\|/.test(tr) || /^(NO\.|NR\.|N°|#)/i.test(tr) || /^\d+\.\d/.test(tr)) break;
    // A heading like "REAKSIE OP TEKS — AFDELING A" or
    // "CAPS Aligned | Barrett's Taxonomy Framework" is duplicate framing.
    if (/^(CAPS|Grade|Graad|REAKSIE|RESPONSE|Barrett)/i.test(tr) || /Framework|raamwerk/i.test(tr)) {
      i++; continue;
    }
    break;
  }
