...
const META_COMMENTARY_RULES = [
  ...
  // exact preamble shape — a sub-question reference followed by colon and
  // a meta-comment about the error report / the correction itself.
  { test: (tr) => /^Q\s*\d+(\.\d+)?\s*:\s+(The\s+(error|working|memo|answer|guidance)|This\s+|No\s+change|The\s+memorandum|The\s+correction)/i.test(tr) },
  { test: (tr) => /^Q\s*\d+(\.\d+)?\s*:\s+(Confirmed|Already correct|Requires no change|Fix:)/i.test(tr) },
 ...
 * Only transforms line-initial labels (^Answer: / ^Working:) — with
 * optional leading whitespace, since some MCQ blocks indent the stub
 * one level under the option list. We deliberately do NOT touch
 * mid-sentence "answer:" inside a passage.
 */
 ...
 */
const MEMO_BANNER = /^\s*(MEMORANDUM|MEMORANDUM\s*[-—:]|MEMO|REAKSIE OP TEKS\s*[-—].*)\s*$/i;
...
export function cleanOutput(text) {
  ...
  const collapsed = [];
  let blanks = 0;
  for (const line of dedupedBanners) {
    if (line.trim() === '') {
      blanks++;
