...
export function cleanOutput(text) {
  ...
  // Pass 4 — stop at the start of a duplicate "COGNITIVE LEVEL …" analysis
  // block. Claude sometimes writes the analysis table twice; we only want
  // the first occurrence.
  const COG_HEADING = /COGNITIVE LEVEL/i;
  const COG_TABLE_ROW = /Prescribed\s*%/i;
  let cogCount = 0;
  const deduped = [];
  for (const line of collapsed) {
    if (COG_HEADING.test(line) || COG_TABLE_ROW.test(line)) {
      cogCount++;
      if (cogCount === 2) break;
    }
    deduped.push(line);
