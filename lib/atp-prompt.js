...
export function buildLessonContextBlock(unit, lessonMinutes, language) {
  ...
  const lines = [
    ...
    `The schema CANNOT enforce these two rules. If you skip either one your tool call will be REJECTED, even if everything else is structurally valid. Live testing has shown both are easy to miss, so treat them as the highest-priority constraints in this entire prompt.`,
    ``,
    `**HARD RULE 1 — exactly ONE phase has worksheetRef:true.**`,
    `In lesson.phases, the FOURTH phase ("${phaseNames[3]}") MUST have \`worksheetRef: true\`. The other four phases MUST OMIT the worksheetRef field entirely (do NOT set it to false; OMIT it). The validator counts phases where worksheetRef === true; that count MUST equal exactly 1.`,
    ``,
    `**HARD RULE 2 — exactly ONE slide has layout:"title", and it is slide #1.**`,
    `In lesson.slides, the slide with the smallest ordinal (typically ordinal:1) MUST have \`layout: "title"\`. NO other slide may have \`layout: "title"\`. All other slides choose from: "objectives", "vocabulary", "concept", "example", "practice", "diagram", "exitTicket". The validator counts slides where layout === "title"; that count MUST equal exactly 1.`,
    ``,
    `BEFORE you emit your tool call, mentally scan your draft:`,
    `  • count phases with worksheetRef:true → must be exactly 1 (on phase #4 "${phaseNames[3]}")`,
    `  • count slides with layout:"title" → must be exactly 1 (on the first slide)`,
    `If either count is wrong, fix the draft before emitting.`,
    ``,
    ...
    `      1. ${phaseNames[0]} — ${intro} minutes  (no worksheetRef)`,
    `      2. ${phaseNames[1]} — ${direct} minutes  (no worksheetRef)`,
    `      3. ${phaseNames[2]} — ${guided} minutes  (no worksheetRef)`,
    `      4. ${phaseNames[3]} — ${indep} minutes  ←  SET worksheetRef: true ON THIS PHASE (Hard Rule 1). The attached worksheet (sections+memo) is what learners do here.`,
    `      5. ${phaseNames[4]} — ${consolid} minutes  (no worksheetRef)`,
    ...
    `      • Slide #1 MUST have \`layout: "title"\` (Hard Rule 2). Heading is the lesson topic (≤8 words). NO other slide may use layout:"title".`,
