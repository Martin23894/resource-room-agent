 ...
 */
export function buildLessonContextBlock(unit, lessonMinutes, language, subtopic = null, seriesContext = null) {
  if (!unit) return [];
  const af = language === 'Afrikaans';
  const focused = !!(subtopic && subtopic.heading);
  const inSeries = !!(seriesContext
    && Number.isInteger(seriesContext.position)
    && Number.isInteger(seriesContext.total)
    && seriesContext.total > 1);
  ...
  // Series context block: when the frontend is orchestrating a multi-
  // lesson series for this Unit, tell the model exactly where this lesson
  // sits in the sequence and what's already been taught. Without this,
  // every series lesson independently picks the most "introductory"
  // angle and they overlap heavily.
  const seriesLines = [];
  if (inSeries) {
    const { position, total, priorSubtopics = [] } = seriesContext;
    seriesLines.push(`### Series context`);
    seriesLines.push(`This is lesson ${position} of ${total} in a teacher-planned series covering the CAPS Unit "${unit.topic}". The teacher will teach all ${total} lessons in order across the Unit's allocated weeks.`);
    if (priorSubtopics.length === 0 && position === 1) {
      seriesLines.push(`This is the FIRST lesson in the series — no prior lessons to build on. Treat this as the Unit's opener: introduce vocabulary, set context, and don't assume learners have prior Unit-specific knowledge beyond the Unit's prerequisites.`);
    } else if (priorSubtopics.length > 0) {
      seriesLines.push(`Prior lessons in this series have already covered:`);
      for (const h of priorSubtopics) seriesLines.push(`  • ${h}`);
      seriesLines.push(`BUILD ON those prior lessons — assume learners already understand those concepts. Do NOT re-teach them. A short recall-prompt in the Introduction phase is fine; a full re-teach is wrong.`);
    }
    if (position < total) {
      seriesLines.push(`Future lessons in this series will cover the remaining subtopics — do NOT pre-empt them in this lesson.`);
    } else if (position === total) {
      seriesLines.push(`This is the LAST lesson in the series — its Consolidation phase should also serve as a brief recap of the whole Unit, and the homework can include a Unit-wide self-check question.`);
    }
    seriesLines.push(``);
  }
  ...
  const lines = [
    ...
    ...seriesLines,
    focused
      ? `You are designing ONE classroom lesson of ${lessonMinutes} minutes within the CAPS Unit "${unit.topic}".${inSeries ? ` This is lesson ${seriesContext.position} of ${seriesContext.total} in the series.` : ''} The lesson focuses NARROWLY on the subtopic "${subtopic.heading}" — not the whole Unit. Other subtopics of this Unit will be taught in separate lessons.`
      : `You are designing ONE classroom lesson of ${lessonMinutes} minutes that teaches the CAPS Unit "${unit.topic}".${inSeries ? ` This is lesson ${seriesContext.position} of ${seriesContext.total} in the series.` : ''}`,
    unit.capsStrand
    ...
    `      • Include exactly ONE objectives-style slide near the start. Use \`layout:"objectives"\` — the renderer turns each bullet into a coloured check-card.`,
    `      • Include ONE vocabulary slide if there are 2 or more key terms. Use \`layout:"vocabulary"\` for a definition-rich slide (renderer pulls term + definition cards from lesson.vocabulary), OR use \`layout:"wordWall"\` for a punchier 2×N grid of just the terms (great for a quick pre-teach moment). Pick one — don't include both.`,
    `      • The MIDDLE of the deck carries the actual teaching. Each teaching slide has a short heading (≤8 words) and 2–6 bullets. NEVER put more than 8 bullets on a slide. Speaker notes go in speakerNotes, NOT in bullets — bullets are what the LEARNER sees, speakerNotes are what the TEACHER says. **Vary the layouts** — don't make every middle slide \`layout:"concept"\`. Pick from:`,
    `          - \`layout:"concept"\` — exposition. A short heading + 2–6 bullets explaining a concept.`,
    `          - \`layout:"example"\` — a worked example as a flat bullet list (problem → working → answer).`,
    `          - \`layout:"workedExample"\` — same purpose as "example" but renders as numbered step cards. **Prefer this over "example"** when the working has 3–5 distinct steps; it's much clearer for learners.`,
    `          - \`layout:"thinkPairShare"\` — a discussion prompt for the class. The heading is a question; bullets (1–3) are sub-prompts. Renders with a paired-figure cue so the teacher knows to put learners in pairs.`,
    `          - \`layout:"diagram"\` — an embedded bar_graph / number_line / food_chain stimulus. Only valid when you have ALSO declared a matching renderable stimulus in stimuli[] and \`stimulusRef\` resolves to it. Do NOT invent a diagram slide whose stimulus you did not declare in stimuli[].`,
    `      • Optionally OPEN with a warm-up: \`layout:"warmUp"\` slide #2 (right after the title) — a single playful prompt or a quick recall question that gets learners thinking. Heading is the question; bullets are optional (use 0–2 bullets to give scaffolding hints).`,
    `      • Include ONE worksheet-cue slide that tells learners to start the worksheet. Use \`layout:"yourTurn"\` for a bright kid-friendly cue (preferred) OR \`layout:"practice"\` for a plainer "Now try these…" header. Heading is action-style ("Try these now", "Your turn", "Probeer hierdie nou" in Afrikaans).`,
    `      • End with EITHER a closing/exit-ticket slide \`layout:"exitTicket"\` carrying the consolidation question(s) OR a \`layout:"celebrate"\` closer that recaps + congratulates (junior-grade-leaning). 1–3 short bullets max either way.`,
    `      • **Variety**: across the whole deck, use **at least 3 different layouts** beyond \`title\`/\`objectives\`/\`vocabulary\` (i.e. the middle + closer should not all be the same layout). A deck of 8 slides where 5 are \`layout:"concept"\` is wrong — break it up with a workedExample, thinkPairShare, or warmUp.`,
