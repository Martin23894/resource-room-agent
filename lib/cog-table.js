...
import { i18n, COG_LEVEL_ALIASES } from './i18n.js';
...
// Section C / Summary memos label rows by content-point rather than
// numbered question — e.g. "Content Point 1", "C.1", "C 1" (English) or
// "Inhoudspunt 1" (Afrikaans). These rows carry real Reorganisation marks
// and must be parsed; without this, every Section C memo's marks vanished
// from the cog-level tally (Resource 5 audit, Bug B).
//
// We accept (case-insensitive):
//   "Content Point 1"   "Content Point 12"
//   "Inhoudspunt 1"     "Inhoudspunt 12"
//   "C.1"   "C 1"   "C1"  (Afrikaans memos sometimes use bare "C.N")
const CONTENT_POINT_RE = /^(?:Content\s+Point|Inhoudspunt|C[.\s]?)\s*\d+$/i;

function isAcceptedRowNumber(cell) {
  if (!cell) return false;
  return ROW_NUMBER_RE.test(cell) || CONTENT_POINT_RE.test(cell);
}
...
export function parseMemoAnswerRows(memoText) {
  ...
  for (const line of memoText.split('\n')) {
    ...
    if (!isAcceptedRowNumber(num)) continue;
...
export function correctCogAnalysisTable(memoText, { levels, totalMarks } = {}) {
  ...
  if (t.totalIdx !== -1) {
    ...
    if (cells) {
      ...
      if (t.actualPctIdx !== -1) {
        // F4 / Bug E (Resource 5): when actual marks don't match the cover
        // total (mark drift), the TOTAL row must reflect the genuine
        // ratio — not a hardcoded 100%. The deterministic emitter
        // (buildCogAnalysisTable line 502) had this fix; the post-correction
        // pass overwrote it because it always wrote 100%. Now both
        // functions agree: 100% only when totalCounted === totalMarks.
        const totalPct = !totalMarks
          ? '0%'
          : totalCounted === totalMarks
            ? '100%'
            : fmtPct(totalCounted, totalMarks);
...
// to one canonical English level. Tries:
//   1. canonical English name (e.g. "Reorganisation")
//   2. language translation (e.g. "Herorganisasie" for Afrikaans)
//   3. registered aliases (e.g. "Reorganisasie") from COG_LEVEL_ALIASES
//
// (3) was the F3 fix: the model occasionally emits an Afrikaans-anglicised
// spelling that isn't the canonical translation. Without alias matching,
// every row at that level vanishes from the tally.
function levelMatcher(canonicalLevel, translations) {
  const normalise = (s) => String(s || '').toLowerCase().replace(/\s+/g, ' ').trim();
  const accepted = new Set();
  accepted.add(normalise(canonicalLevel));
  const tr = translations?.[canonicalLevel];
  if (tr) accepted.add(normalise(tr));
  for (const a of COG_LEVEL_ALIASES?.[canonicalLevel] || []) {
    accepted.add(normalise(a));
  }
  accepted.delete('');
  return (rowLabel) => accepted.has(normalise(rowLabel));
}

/**
 * Normalise the COGNITIVE LEVEL column values in a memo's answer table
 * to a single display language. The audit (Resource 2) caught a memo
 * whose Section A used English level names ("Literal", "Inferential")
 * while Section D used Afrikaans ("Letterlik", "Inferensieel"). Both
 * Section bodies are produced by the same Claude call, but the writer
 * sometimes picks one language for one section and the other for
 * another even when the prompt is unambiguous.
 *
 * The fix is post-processing: for each answer-table row, recognise the
 * cog level (case- and language-insensitively, against either the
 * canonical English or its Afrikaans translation) and rewrite the cell
 * to the language's canonical display label.
 *
 * @param {string} memoText
 * @param {object} args
 * @param {string[]} args.canonicalLevels  Canonical English level names
 *                                         (cog.levels from getCogLevels).
 * @param {Record<string, string>} [args.translations]  Map of canonical
 *                                         → display-language label.
 * @param {Array<Record<string, string>>} [args.aliases]  Extra alias maps
 *                                         (canonical → label) to recognise
 *                                         as input. Useful for normalising
 *                                         to English while still matching
 *                                         Afrikaans cells, or vice versa.
 * @returns {{ text: string, changed: boolean, rewrites: number }}
 */
export function normaliseCogLevelLabels(memoText, { canonicalLevels = [], translations = {}, aliases = [] } = {}) {
  if (typeof memoText !== 'string' || memoText.length === 0) {
    return { text: memoText, changed: false, rewrites: 0 };
  }
  const norm = (s) => String(s || '').toLowerCase().replace(/\s+/g, ' ').trim();
  // Build a lookup: any normalised label (English, target translation,
  // extra alias map, or COG_LEVEL_ALIASES variant) → canonical key.
  const lookup = new Map();
  for (const canon of canonicalLevels) {
    lookup.set(norm(canon), canon);
    const tr = translations[canon];
    if (tr) lookup.set(norm(tr), canon);
    for (const aliasMap of aliases) {
      const a = aliasMap?.[canon];
      if (a) lookup.set(norm(a), canon);
    }
    // Recognition-only aliases registered in i18n. Resource 3: an Afrikaans
    // memo wrote "Reorganisasie" that the matcher must rewrite to whatever
    // the target language calls it (Herorganisasie for af, Reorganisation
    // for en).
    for (const a of COG_LEVEL_ALIASES?.[canon] || []) {
      lookup.set(norm(a), canon);
    }
  }
  if (lookup.size === 0) return { text: memoText, changed: false, rewrites: 0 };
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    const m = parseInt(mark, 10);
    if (!Number.isFinite(m) || m < 0) continue;
    const canon = lookup.get(norm(level));
    if (!canon) continue;
    const display = translations[canon] || canon;
    if (level === display) continue;
    cells[3] = display;
    lines[i] = '| ' + cells.join(' | ') + ' |';
    rewrites++;
  }
  return { text: lines.join('\n'), changed: rewrites > 0, rewrites };
}
...
export function buildCogAnalysisTable({
  ...
  // Numbered rows render with a Q prefix ("Q1.1 (3)"); content-point rows
  // render verbatim ("Content Point 1 (1)") — adding "Q" to a non-numeric
  // label produces "QContent Point 1" which is readable but ugly.
  const isNumericRow = (n) => /^\d/.test(String(n || '').trim());
  const formatItem = (it) => isNumericRow(it.number)
    ? `Q${it.number} (${it.mark})`
    : `${it.number} (${it.mark})`;
  const breakdownLines = rows.map((r) => {
    ...
    const items = r.items.map(formatItem).join(' + ');
