...
export function rebalanceMarks(resource, opts = {}) {
  ...
  const { prescribedSectionMarks } = opts;
  ...
  const sectionAdjustments = phase4SectionTargets(resource, prescribedSectionMarks, answerByNum);
...
const SECTION_TOLERANCE = 1;

function phase4SectionTargets(resource, prescribed, answerByNum) {
  ...
  for (let iter = 0; iter < MAX_SECTION_ITER; iter++) {
    ...
    if (overGap <= SECTION_TOLERANCE && underGap >= -SECTION_TOLERANCE) break;
