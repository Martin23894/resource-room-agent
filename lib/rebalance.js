...
function stripMarkAssertions(text) {
  if (!text) return '';
  let s = text;
  // English + Afrikaans number-word matcher 1-10. Live moderation caught
  // the model writing "Award one mark per region, up to two marks" — the
  // digit-only regex below silently misses these, leaving the contradicting
  // count text in place after a rebalance. Word-form coverage closes the
  // most common author phrasings.
  const NW_EN = '(?:one|two|three|four|five|six|seven|eight|nine|ten)';
  const NW_AF = '(?:een|twee|drie|vier|vyf|ses|sewe|agt|nege|tien)';
  // Count token: digits, fractions (½, ¼, ¾), half-words, or a number word.
  const N = `(?:\\d+(?:\\.\\d+)?|[½¼¾⅓⅔]|${NW_EN}|${NW_AF})`;
  // "Award N mark(s) ... ." — drop the entire mark-allocation sentence.
  // Going to the next sentence-ending period/semicolon means we don't
  // leave awkward "For X." / "Per Y." remnants. The criterion content
  // (what to look for) typically lives in a SEPARATE following sentence
  // ("Accept common names used in CAPS textbooks."), so dropping the
  // whole Award sentence rarely removes useful guidance.
  s = s.replace(new RegExp(`\\bAward(?:\\s+up\\s+to)?(?:\\s+a\\s+further)?(?:\\s+full)?\\s+${N}\\s+marks?[^.;\\n]*[.;]?\\s*`, 'gi'), '');
  s = s.replace(new RegExp(`\\b(?:Ken|Toeken)\\s+${N}\\s+punte?\\s+toe[^.;\\n]*[.;]?\\s*`, 'gi'), '');
  // "Up to N marks" / "Tot en met N punte" — strip BEFORE the bare-count
  // pattern so its longer prefix matches first (otherwise the bare-count
  // pattern eats "N punte" and leaves an orphan "tot en met").
  s = s.replace(new RegExp(`\\b(?:up\\s+to|tot\\s+en\\s+met)\\s+${N}\\s+(?:marks?|punte?)\\b\\s*`, 'gi'), '');
  ...
  s = s.replace(new RegExp(`\\b${N}\\s+punte?\\s+(?=vir|per|elke|indien|wanneer)\\b`, 'gi'), '');
...
// Exposed for tests — pure helpers with no rebalancer state.
export const __internals = { stripMarkAssertions, reconcileMarkingGuidance };
